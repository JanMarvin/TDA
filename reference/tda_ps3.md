...
emit `rotate` and `scale`, which [`tda_read_ps`](tda_read_ps.md) does
not yet interpret, so the replayed picture may be positioned wrongly
even where the PostScript itself is correct.
