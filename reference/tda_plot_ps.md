...
Replays what [`tda_read_ps`](tda_read_ps.md) parsed onto the current
grid device, so a plot TDA produced appears in an R graphics device
rather than only in a `.ps` file.
...
- x:
  ...
  a path, a TDA run, or the result of [`tda_read_ps`](tda_read_ps.md).
...
[`tda_pl_scatter()`](tda_pl_scatter.md), [`tda_ps()`](tda_ps.md),
