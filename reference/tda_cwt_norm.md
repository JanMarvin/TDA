...
the curvature at the optimum, not where the optimum is).
`cwt(wnorm=s)=W;` on the raw weights and plain `cwt=W;` on
`tda_cwt_norm(W, s)` give the same fit, to the last printed digit.
