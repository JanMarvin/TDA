...
# plot3.cf's and plot4.cf's own plots as ordinary tda_ps() sessions,
...
p3 <- tda_pl_grid(p3, at_x = 0.5, at_y = c(-1, 0, 1))
p3 <- tda_pl_frame(p3)
p3 <- tda_pl_text(p3, "Linear Coordinate System", at = c(0, 1.15))
f3 <- tda_ps_file(p3)

# plot4.cf: a plain logarithmic coordinate system, no data.
p4 <- tda_ps(xlim = c(1, 1000), ylim = c(2, 500), width = 90, height = 50,
             log = "xy")
p4 <- tda_pl_axes(p4, sc = c(10, 5), ic = 0)
p4 <- tda_pl_grid(p4, at_x = c(10, 50, 250), at_y = c(10, 100, 1000))
p4 <- tda_pl_frame(p4)
p4 <- tda_pl_text(p4, "Logarithmic Coordinate System", at = c(31.6, 650))
f4 <- tda_ps_file(p4)
