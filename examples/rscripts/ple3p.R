...
# also use) and tda_ple() (TDA's own real product-limit estimator, a
# genuine multi-state/competing-risks fit here: three destination
...
to_step <- function(b, tmax = 300) {
    ...
    # TDA's own ple.3 output (and this survivor function generally)
    # holds its value flat from the last observed event until the end
    # of the plot's own range, not just to the last event time --
    # confirmed directly against TDA's own real output for this file,
    # which draws every group's own line flat out to Time=300, not
    # stopping wherever that group's own last event happened to be.
    if (max(d$Time) < tmax)
...
p <- tda_pl_frame(p)
p <- tda_pl_lines(p, "Time", "S", group = "upward", by = "grp", lty = 1)
p <- tda_pl_lines(p, "Time", "S", group = "lateral", by = "grp", lty = 5)
p <- tda_pl_lines(p, "Time", "S", group = "downward", by = "grp", lty = 8)
