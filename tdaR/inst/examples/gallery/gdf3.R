...
# binary -- checked, running the shipped, unmodified file:
# the gdf() computation itself succeeds (5 iterations, 100 records
# written to its output file), but the final plot(s=17,...)=X,Y
...
# file and its specific random data, not something tda_gdf() (a
...
# capability -- gdf3.cf's core idea, a distribution fitted to
# right-censored data via TDA's gdf() -- on data structured the
# same way (a linear trend with random censoring above a threshold),
# using tda_gdf()'s documented, working "distribution" output
