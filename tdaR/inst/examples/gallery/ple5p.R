...
# use, rrdat.1) and tda_ple() (TDA's product-limit/Kaplan-Meier
...
# TDA's real computation nor TDA's real data. tda_ple() is a
# documented, exported function that runs the actual TDA binary's
# ple() command; there is no reason to reimplement it.
#
# ple5p.cf itself connects ple()'s output rows directly (Time,
...
# sharp right angles. Checked against TDA's real
...
# is what TDA's plot(...) actually draws, not an improvement on it.
#
...
# band's white lower-bound fill (TDA's shading technique, see
# .pl_series()'s comment: an upper-bound fill in grey, a
...
# ple5p.cf's selection: sel = T1 = c1[0] & lt(c3,290) -- this
...
p <- tda_pl_lines(p, "Time", "G", select = "Men", by = "grp", lty = 1,
                  band = c("UBnd", "LBnd"))
p <- tda_pl_lines(p, "Time", "G", select = "Women", by = "grp", lty = 5,
                  band = c("UBnd", "LBnd"))
p <- tda_pl_frame(p)
