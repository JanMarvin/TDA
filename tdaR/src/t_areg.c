...
int npreg1(TDAContext *ctx, int opt,int kopt,int n,double *x,double *y,int nx,double *xx,double d);
void npreg2(TDAContext *ctx, int opt,int n,int ny,int *y);
double mkern(TDAContext *ctx, int opt,int n,double d,double a,double *x,double *y);
double mkern1(TDAContext *ctx, int opt,int n,double d,double a,double *x);
int lowess(TDAContext *ctx, double *x,double *y,int n,double f,int nsteps,double delta, double *ys,double *rw,double *res);
int lowest(TDAContext *ctx, double *x,double *y,int n,double xs,double *ys,int nleft, int nright,double *w,int userw,double *rw);
int rmidmean(TDAContext *ctx, int n,double *x,double *y,int r,double *xm,double *ym, float *ym1,float *ym2);
double midmean(TDAContext *ctx, int n,double *x,int opt,int *err);
int smidmean(TDAContext *ctx, int n,double *x,double *sm1,double *sm2);
int inpreg(TDAContext *ctx);
double imkern(TDAContext *ctx, int opt,int n,double d,double a,double *x,double *y);
...
int npreg(TDAContext *ctx)
{
    ...
    if (check_cmd(ctx, 1))
        return(-1);
    /*  prnchar('-',LLEN,1); */
    printf1(ctx, "Nonparametric regression. Current memory: %d bytes.\n",ctx->MemReq);
    ...
    if (ctx->PMNTP < 1) {
        printf1(ctx, "Error: need points defined with x parameter.\n");
        goto NPRFin;
    }
    if (ctx->PMD < ctx->EPSI1)
        ctx->PMD = 1.0;

    if (!ctx->PMF1Def) {             /* need an output file */
        p_err(ctx, -27,1);
        goto NPRFin;
    }
    ...
    nrec = 0;
    if (ctx->PMOPT <= 3) {           /* means */
        printf1(ctx, "smoothed ");
        if (ctx->PMOPT == 1)  
            printf1(ctx, "means");
        else if (ctx->PMOPT == 2)
            printf1(ctx, "quantiles");
        else            
            printf1(ctx, "frequencies");

        printf1(ctx, ". Interval width: %lg\n",ctx->PMD);
        if (ctx->PMOPT == 1) {
            printf1(ctx, "Kernel: ");
            switch (ctx->PMK) {
                case 2: printf1(ctx, "triangle\n"); break;
                case 3: printf1(ctx, "quartic\n"); break;
                case 4: printf1(ctx, "Epanechnikov\n"); break;
                default: ctx->PMK = 1; printf1(ctx, "uniform\n"); break;
            }
        }
        nrec = npreg1(ctx, ctx->PMOPT,ctx->PMK,nn,ctx->AcX,ctx->AcY,ctx->PMNTP,ctx->PMTP,ctx->PMD);
    }
    else if (ctx->PMOPT == 4) {           /* lowess */
        ...
        if (ctx->PMD < 0.0)
            ctx->PMD = 0.0;
        printf1(ctx, "Lowess: sig=%lg ns=%d d=%lg\n",ctx->PMSIG,ctx->PMNS,ctx->PMD);
        ...
        for (i = 1; i <= nn; ++i) {
            ...
            rt_fprintf_d(ctx, ctx->PMF1d,ctx->PMFmtS,ctx->AcX[i - 1]);
            rt_fprintf_d(ctx, ctx->PMF1d,ctx->PMFmtS,ctx->AcY[i - 1]);
...
#ifdef TDA_R_PACKAGE
            {
                ...
                erow[2] = ctx->AcY[i - 1];
                erow[3] = ctx->AcU[i];
                tda_export_row(ctx, "npreg.table", erow, 4);
            }
#endif
        }
#ifdef TDA_R_PACKAGE
        tda_export_flush(ctx, "npreg.table");
#endif
        printf1(ctx, "%d records written to: %s\n",nrec,ctx->PMF1dName);
    }
    else if (ctx->PMOPT == 5) {           /* midmeans */
        ...
        /**
        for (i = 0; i < nn; ++i) {
            nneighbor(ctx, nn,AcX,PMR,i,&k1,&k2);
            AxU[i] = midmean(ctx, PMR,AcX + k1,0,&err);

            for (j = 0; j < PMR; ++j)
                AcTmp[j] = AcY[j + k1];

            AcV[i] = midmean(ctx, PMR,AcTmp,1,&err);
        }
        **/
    }
    err = 0;
...
int npreg1(TDAContext *ctx, int opt,int kopt,int n,double *x,double *y,int nx,double *xx,double d)
{
    ...
    if (sortd2(ctx, n,x,y))     /* sort simultaneously according to x */
        return(-1);       

    if (alloc_acu(ctx, n + 1))
        return(-1);  
    if (alloc_acv(ctx, n + 1))
        return(-1);         
    ...
    if (opt == 2) {
        ...
        if (sortd(ctx, n,ctx->AcU,0))   
            return(-1);       

        nycat = 1;
        ctx->AcN[0] = (int)ctx->AcU[0];
        i = 0;
        while (++i < n) {
            if (ctx->AcU[i] != ctx->AcU[i - 1])
                ctx->AcN[nycat++] = (int)ctx->AcU[i];
        }
        printf1(ctx, "Number of categories in dependent variable: %d\nCategories:",nycat);
        for (i = 0; i < nycat; ++i)
            printf1(ctx, " %d",ctx->AcN[i]);
        newline(ctx);     
         
        if (alloc_acm(ctx, nycat))
            return(-1);  
    }
    ...
    for (j = 0; j < nx; ++j) {
        ...
        xm = xmean(ctx, m,ctx->AcU,ctx->AcW);

        fprintf(ctx->PMF1d,"%6d ",j + 1);
        rt_fprintf_d(ctx, ctx->PMF1d,ctx->PMFmtS,xx[j]);
        fprintf(ctx->PMF1d,"%6d ",m);
        rt_fprintf_d(ctx, ctx->PMF1d,ctx->PMFmtS,xm);   /* x mean */
#ifdef TDA_R_PACKAGE
        /* the row's width depends on opt (mean+sd, one column per
           quantile, or one per category), so it is accumulated cell by
           cell and committed where the newline goes */
        tda_export_cell(ctx, "npreg.table", (double)(j + 1));
        tda_export_cell(ctx, "npreg.table", xx[j]);
        tda_export_cell(ctx, "npreg.table", (double)m);
        tda_export_cell(ctx, "npreg.table", xm);
#endif

        if (opt == 1) {
            ym = xstd(ctx, m,ctx->AcV,ctx->AcW,&ys);

            if (kopt > 1)
                ym = mkern(ctx, kopt,m,d,xx[j],ctx->AcU,ctx->AcV);

            rt_fprintf_d(ctx, ctx->PMF1d,ctx->PMFmtS,ym);   /* y mean */
            rt_fprintf_d(ctx, ctx->PMF1d,ctx->PMFmtS,ys);   /* y standard deviation */
#ifdef TDA_R_PACKAGE
            tda_export_cell(ctx, "npreg.table", ym);
            tda_export_cell(ctx, "npreg.table", ys);
#endif
        }
        else if (opt == 2) {
            if (m > 1) {
                if (sortd(ctx, m,ctx->AcV,0))
                    return(-1);    
            }
            for (i = 0; i < nqx; ++i) {
                ym = quantf(ctx, m,ctx->AcV,qx[i]);
                rt_fprintf_d(ctx, ctx->PMF1d,ctx->PMFmtS,ym); 
#ifdef TDA_R_PACKAGE
                tda_export_cell(ctx, "npreg.table", ym);
#endif
            }
            ...
            for (k = 0; k < nycat; ++k) { 
                rt_fprintf_d(ctx, ctx->PMF1d,ctx->PMFmtS,(double)ctx->AcM[k] / (double)m);
#ifdef TDA_R_PACKAGE
                tda_export_cell(ctx, "npreg.table",
                                (double)ctx->AcM[k] / (double)m);
#endif
            }
        }
        fprintf(ctx->PMF1d,"\n");
#ifdef TDA_R_PACKAGE
        tda_export_endrow(ctx, "npreg.table");
#endif
        nrec++;
    }
#ifdef TDA_R_PACKAGE
    ...
    printf1(ctx, "%d records written to: %s\n",nrec,ctx->PMF1dName);
    if (ctx->PMTDAFDef)  
        npreg2(ctx, opt,nrec,nycat,ctx->AcN);
...
void npreg2(TDAContext *ctx, int opt,int n,int ny,int *y)
{
    ...
    fprintf(ctx->PMTDAFd,"# data written by npreg command.\n");
    fprintf(ctx->PMTDAFd,"nvar(\n");
    fprintf(ctx->PMTDAFd,"  dfile = %s,\n",ctx->PMF1dName);
    fprintf(ctx->PMTDAFd,"  noc = %d,\n",n);
    k = 0;
    fprintf(ctx->PMTDAFd,"  RECN [ 6.0] = c%-2d, # record number\n",++k);
    fprintf(ctx->PMTDAFd,"  X    [%d.%d] = c%-2d, # x value\n",ctx->PMFmt1,ctx->PMFmt2,++k);
    fprintf(ctx->PMTDAFd,"  NX   [ 6.0] = c%-2d, # number of cases\n",++k);
    fprintf(ctx->PMTDAFd,"  XM   [%d.%d] = c%-2d, # mean of X\n",ctx->PMFmt1,ctx->PMFmt2,++k);

    switch (opt) {
        case 1:
            fprintf(ctx->PMTDAFd,"  YM   [%d.%d] = c%-2d, # mean of Y\n",ctx->PMFmt1,ctx->PMFmt2,++k);
            fprintf(ctx->PMTDAFd,"  YSD  [%d.%d] = c%-2d, # standard deviation of Y\n",ctx->PMFmt1,ctx->PMFmt2,++k);
            break;
        ...
        case 2:
            ...
            for (i = 0; i < 11; ++i)  
                fprintf(ctx->PMTDAFd,"  Q%2d  [%d.%d] = c%-2d, # .%2d quantile of Y\n",q[i],ctx->PMFmt1,ctx->PMFmt2,++k,q[i]);
            break;

        case 3:
            for (i = 0; i < ny; ++i)  
                fprintf(ctx->PMTDAFd,"  Y%-4d[%d.%d] = c%-2d, # frequency of Y=%d\n",y[i],ctx->PMFmt1,ctx->PMFmt2,++k,y[i]);
            break;
...
int lowess(TDAContext *ctx, double *x,double *y,int n,double f,int nsteps,double delta, double *ys,double *rw,double *res)
{
    ...
    ns = imax(ctx, imin(ctx, (int)(f * (double)n),n),2);
    ...
    for (iter = 1; iter <= nsteps + 1; iter++) {    /* robustness iterations */
        ...
        while (1) {
            ...
            ok = lowest(ctx, x,y,n,x[i],&ys[i],nleft,nright,res,userw,rw);
...
int lowest(TDAContext *ctx, double *x,double *y,int n,double xs,double *ys,int nleft, int nright,double *w,int userw,double *rw)
{
    ...
    ok = 0;
    range = x[n] - x[1];
    h = dmax(ctx, xs - x[nleft],x[nright] - xs);
    h9 = 0.999 * h;
    h1 = 0.001 * h;
...
int rmidmean(TDAContext *ctx, int n,double *x,double *y,int r,double *xm,double *ym, float *ym1,float *ym2)
{
    ...
    for (i = 0; i < n; ++i) {
        ...
        xm[i] = midmean(ctx, r,x + k1,0,&err);
        if (err)
            break;

        for (j = 0; j < r; ++j)
            ctx->AcTmp[j] = y[j + k1];
        ym[i] = midmean(ctx, r,ctx->AcTmp,1,&err);
        if (err)
            break;

        for (j = 0; j < r; ++j)
            ctx->AcTmp[j] = y[j + k1] - ym[i];
        err = smidmean(ctx, r,ctx->AcTmp,&sm1,&sm2);
        if (err)
            break;
...
double midmean(TDAContext *ctx, int n,double *x,int opt,int *err)
{
    ...
    *err = 0;
    ...
    qx1 = quantf(ctx, n,x,0.25);
    qx2 = quantf(ctx, n,x,0.75);
...
int smidmean(TDAContext *ctx, int n,double *x,double *sm1,double *sm2)
{
    ...
    qx1 = quantf(ctx, n,x,1.0 / 8.0);
    qx2 = quantf(ctx, n,x,3.0 / 8.0);
    ...
    *sm1 = tmp;
    ...
    qx1 = quantf(ctx, n,x,5.0 / 8.0);
    qx2 = quantf(ctx, n,x,7.0 / 8.0);
...
int inpreg(TDAContext *ctx)
{
    ...
    printf1(ctx, "Nonparametric regression with interval-valued variables.\n");
    ...
    if (alloc_acx(ctx, ctx->NOC + 1))     /* x lower bound */ 
        goto INPFin;
    if (alloc_acv(ctx, ctx->NOC + 1))     /* x upper bound */ 
        goto INPFin;
    if (alloc_acy(ctx, ctx->NOC + 1))     /* y lower bound */ 
        goto INPFin;
    if (alloc_acw(ctx, ctx->NOC + 1))     /* y upper bound */
        goto INPFin;

    if (alloc_actmp(ctx, ctx->NOC + 1))                       
        goto INPFin;
    if (alloc_actmp1(ctx, ctx->NOC + 1))                       
        goto INPFin;
    ...
    xlm = xhm = ylm = yhm = 0.0;
    for (i = 0; i < ctx->NOC; ++i) {
        xl = get_data(ctx, ixl,i);
        xh = get_data(ctx, ixh,i);
        yl = get_data(ctx, iyl,i);
        yh = get_data(ctx, iyh,i);
        if (yh < yl) {
    ...
    printf1(ctx, "Smoothing means, kernel: ");
    switch (ctx->PMK) {
        case 2: printf1(ctx, "triangle\n"); break;
        case 3: printf1(ctx, "quartic\n"); break;
        case 4: printf1(ctx, "Epanechnikov\n"); break;
    ...
    printf1(ctx, "Number of cases: %d\n",ctx->NOC);
    printf1(ctx, "Range of x means: ");
    rt_printf1_d(ctx, ctx->PMFmtS,xlm);
    rt_printf1_d(ctx, ctx->PMFmtS,xhm);
    printf1(ctx, "\nRange of y means: ");
    rt_printf1_d(ctx, ctx->PMFmtS,ylm);
    rt_printf1_d(ctx, ctx->PMFmtS,yhm);
    newline(ctx);
/*
    for (i = 0; i < NOC; ++i)
        printf1(ctx, "i=%d xl=%lf xh=%lf yl=%lf yh=%lf\n",i,AcX[i],AcV[i],AcY[i],AcW[i]);         
*/
    w = ctx->PMD / 2.0;
    ...
    for (j = 0; j < ctx->PMNTP; ++j) {
        ...
        ylm = yhm = 0.0;
        for (i = 0; i < ctx->NOC; ++i) {
            if (ctx->AcX[i] <= b && ctx->AcV[i] >= a) {
                ylm += ctx->AcY[i];
                yhm += ctx->AcW[i];
                ctx->AcTmp[m] = ctx->AcY[i];
                ctx->AcTmp1[m] = ctx->AcW[i];
                ctx->AcT[m] = ctx->AcX[i];
                ctx->AcU[m] = ctx->AcV[i];
                m++;
            }
...
/**
        tda_out("m=%d ",m);
        for (i = 0; i < m; ++i)
            tda_out(" (%lg,%lg) ",AcTmp[i],AcTmp1[i]);
        newline(ctx);

        tda_out("m=%d ",m);
        for (i = 0; i < m; ++i)
            tda_out(" (%lg,%lg) ",AcT[i],AcU[i]);
        newline(ctx);
**/
        if (m == 0)
            continue;
        ...
        for (iter = 0; iter < ctx->MxIter; ++iter) {
            ...
            for (i = 0; i < m; ++i) {
                if (ctx->AcTmp[i] <= ylm) {
                    ...
                    if (fabs(x - ctx->AcT[i]) >= fabs(x - ctx->AcU[i]))
                        ctx->AcZ[i] = ctx->AcT[i];
                    else
                        ctx->AcZ[i] = ctx->AcU[i];
                }
...
/**
            ...
            newline(ctx);
**/
            ym = imkern(ctx, ctx->PMK,m,ctx->PMD,x,ctx->AcZ,ctx->AcTmp);

/*          tda_out("ym=%lf \n",ym); */

            if (ym >= ylm - ctx->EPSI1) {
                ylm = ym;
/*   tda_out("break ym=%lg ylm=%lg\n",ym,ylm);   */
                break;
            }
            ylm = ym;
        }
        itmax = imax(ctx, itmax,iter);
...
/*
            ...
            newline(ctx);
*/
            ym = imkern(ctx, ctx->PMK,m,ctx->PMD,x,ctx->AcZ,ctx->AcTmp1);
...
/*          tda_out("ym=%lf \n",ym); */
        ...
        rt_fprintf_d(ctx, ctx->PMF1d,ctx->PMFmtS,ylm);                 
        rt_fprintf_d(ctx, ctx->PMF1d,ctx->PMFmtS,yhm);                 
        fprintf(ctx->PMF1d,"\n");
#ifdef TDA_R_PACKAGE
        {
            double erow[4];
            ...
            erow[2] = ylm;
            erow[3] = yhm;
            tda_export_row(ctx, "ivreg.bounds", erow, 4);
        }
#endif
        nrec++;
    }
#ifdef TDA_R_PACKAGE
    tda_export_flush(ctx, "ivreg.bounds");
#endif
    printf1(ctx, "\n%d records written to: %s\n",nrec,ctx->PMF1dName);
    printf1(ctx, "Max number of iterations used: %d\n",itmax);
