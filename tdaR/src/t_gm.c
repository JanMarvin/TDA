...
void g_spat(TDAContext *ctx, double ax,double ay,double az,double bx,double by,double bz, double *rx,double *ry,double *rz);
int g_intersect(TDAContext *ctx, double xia,double yia,double xib,double yib, double xja,double yja,double xjb,double yjb,double *xxa,double *yya, double *xxb,double *yyb);
...
int g_isect(TDAContext *ctx, double x1,double y1,double x2,double y2,double x3,double y3, double x4,double y4);
...
void g_pol_rect(TDAContext *ctx, int n,double *x,double *y,double *xmin,double *xmax, double *ymin,double *ymax);
int g_inpoly(TDAContext *ctx, int n, double *x,double *y,double xa,double ya);
int g_l_inpoly(TDAContext *ctx, int n, double *x,double *y,double xa,double ya,double xb, double yb);
...
int g_pol_isect(TDAContext *ctx, int n, double *x,double *y,double xa,double ya, double xb,double yb);
int g_chull(TDAContext *ctx, float *x,float *y,int m,int *in,int *ih,int *il);
void g_chull_split(TDAContext *ctx, float *x,float *y,int m,int *in,int ii,int jj,int s, int *iabv,int *na,int *maxa,int *ibel,int *nb,int *maxb);
void g_xchxy(TDAContext *ctx, double *x,double *y);
double g_len(TDAContext *ctx, double xa,double ya,double xb,double yb);
double g_vlen(TDAContext *ctx, double x,double y);
void g_poly_ccw(TDAContext *ctx, int n,double *x,double *y);
int g_line_intersect(TDAContext *ctx, double p1x,double p1y,double p2x,double p2y, double q1x,double q1y,double q2x,double q2y, double *s1x,double *s1y,double *s2x,double *s2y,double eps);
...
int g_intersect(TDAContext *ctx, double xia,double yia,double xib,double yib, double xja,double yja,double xjb,double yjb,double *xxa,double *yya, double *xxb,double *yyb)
{
    double ux,uy,vx,vy,wx,wy,d,du,dv,w2x,w2y,t,t0,t1,s1;
    ...
    wx = xia - xja;
    wy = yia - yja;
    d = g_perp(ctx, ux,uy,vx,vy);

    /*  test if segments are parallel (includes either being a point) */

    if (fabs(d) < ctx->EPSI1) {       /* parallel */
        if (g_perp(ctx, ux,uy,wx,wy) != 0.0 || g_perp(ctx, vx,vy,wx,wy) != 0.0) /* not collinear */
            return(0);
        ...
        if (du == 0.0) {                /* first segment is single point */
            ...
            if (g_inseg(ctx, xia,yia,xja,yja,xjb,yjb) == 0)
                return(0);
        ...
        if (dv == 0.0) {                /* second segment is single point */
            ...
            if (g_inseg(ctx, xja,yja,xia,yia,xib,yib) == 0)
                return(0);
...
int g_isect(TDAContext *ctx, double x1,double y1,double x2,double y2,double x3,double y3, double x4,double y4)
{
    if (g_ccw(ctx, x1,y1,x2,y2,x3,y3) * g_ccw(ctx, x1,y1,x2,y2,x4,y4) <= 0 &&
        g_ccw(ctx, x3,y3,x4,y4,x1,y1) * g_ccw(ctx, x3,y3,x4,y4,x2,y2) <= 0)       
        return(1);
    return(0);
}
...
double g_pol_xmin(TDAContext *ctx, int n,double *x,double *y)
{
    (void)ctx; (void)y;        /* unused: the signature is shared */
    register int i;
    double xmin;
...
double g_pol_ymin(TDAContext *ctx, int n,double *x,double *y)
{
    (void)ctx; (void)x;        /* unused: the signature is shared */
    register int i;
    double ymin;
...
int g_l_inpoly(TDAContext *ctx, int n, double *x,double *y,double xa,double ya,double xb, double yb)
{
    ...
    if (g_inpoly(ctx, n,x,y,xa,ya) == 0)
        return(0);
    if (g_inpoly(ctx, n,x,y,xb,yb) == 0)
        return(0);
    ...
    for (i = 0; i < n; ++i) {
        xnew = x[i];
        ynew = y[i];
        if (g_intersect(ctx, xa,ya,xb,yb,xold,yold,xnew,ynew,&xxa,&yya,&xxb,&yyb) == 1) {
            if ((fabs(xa - xxa) > ctx->EPSI1 || fabs(ya - yya) > ctx->EPSI) &&
                (fabs(xb - xxa) > ctx->EPSI1 || fabs(yb - yya) > ctx->EPSI) &&
                (fabs(xold - xxa) > ctx->EPSI1 || fabs(yold - yya) > ctx->EPSI) &&
                (fabs(xnew - xxa) > ctx->EPSI1 || fabs(ynew - yya) > ctx->EPSI))  
                return(0);
        }
        xold = xnew;
        yold = ynew;
...
int g_p_inpoly(TDAContext *ctx, int na, double *xa,double *ya,int nb,double *xb,double *yb)
{
    ...
    for (i = 1; i < na; ++i) {
        if (g_l_inpoly(ctx, nb - 1,xb,yb,xa[i-1],ya[i-1],xa[i],ya[i]) == 0)
            return(0);
    }
    return(1);
} 
...
int g_pol_checkp(TDAContext *ctx, int n,double *x,double *y,double px,double py)
{
    ...
    for (i = 1; i <= n; ++i) {
        p2x = x[i % n];
        p2y = y[i % n];
        if (py > dmin(ctx, p1y,p2y)) {
            if (py <= dmax(ctx, p1y,p2y)) {
                if (px <= dmax(ctx, p1x,p2x)) {
                    if (p1y != p2y) {
...
int g_chull(TDAContext *ctx, float *x,float *y,int m,int *in,int *ih,int *il) 
{
    ...
    if (!(ia = (int *)calloc((size_t)(m + 1),sizeof(int))))
        return(-1);       

    if (!(ib = (int *)calloc((size_t)(m + 1),sizeof(int)))) {
        free((char *)ia);
        return(-1);       
    }
    if (m == 1)          
...
CONV7:
    ...
    g_chull_split(ctx, x,y,mm,in,ih[1],ih[2],0,ia,&mb,&mxa,ib,&ia[ma],&mxbb);
...
CONV9:  
        ...
        g_chull_split(ctx, x,y,mb,ia,ih[inh],ih[ilinh],1,ia,&mbb,&mxa,ib + nib - 1,&ia[ma],&mxb);
        mb = mbb;
    }
    ...
    while (1) {
        ...
        if (ia[ma] != 0) {           
            ...
            g_chull_split(ctx, x,y,ia[ma],ib + nib - 1,ih[inh],ih[ilinh],2,ia,&mb,&mxa,ib + nib - 1,&mbb,&mxb);
            ia[ma] = mbb;
            goto CONV9;
        }
    }             
...
CONV14: 
        ...
        g_chull_split(ctx, x,y,mb,ib + nib - 1,ih[inh],ih[ilinh],-1,ia + nia - 1,&ia[ma],&mxa,ib + nib - 1,&mbb,&mxb);
        mb = mbb;
    }
    ...
    while (1) {
        inh = il[inh];
        ma++;   
        /*  ia is calloc(m + 1), so m is the last valid index, and mp1 is
            m + 1.  Reading ia[ma] BEFORE the test meant that on the
            final pass -- ma == mp1 -- it read one int past the end.
            The test moves ahead of the read; nia is not used after the
            jump, so nothing else changes.  */
        if (ma == mp1)          
            goto CONVFin;
        nia -= ia[ma];
    ...
    g_chull_split(ctx, x,y,ia[ma],ia + nia - 1,ih[inh],ih[ilinh],-2,ia + nia - 1,&mbb,&mxa,ib + nib - 1,&mb,&mxb);
    goto CONV14;
...
void g_chull_split(TDAContext *ctx, float *x,float *y,int m,int *in,int ii,int jj,int s, int *iabv,int *na,int *maxa,int *ibel,int *nb,int *maxb)
{
    ...
    *maxb = 0;
    ...
    if (x[jj] != x[ii]) {           
        a = (double)((y[jj] - y[ii]) / (x[jj] - x[ii]));
        b = (double)(y[ii]) - (double)(a) * (double)(x[ii]);
    }
    else {
        xt = (double)(x[ii]);
        if (y[jj] < y[ii])
    ...
    for (i = 1; i <= m; ++i) {
        is = in[i];
        if (t)           
            z = dir * ((double)(x[is]) - xt);
        else  
            z = (double)(y[is]) - (double)(a) * (double)(x[is]) - b;
...
#define Vadds(r,a,b,s)  {(r)[0] = (double)((((((((a)[0] + (b)[0]))))))) * (double)((s)); (r)[1] = (double)((((((((a)[1] + (b)[1]))))))) * (double)((s));}

#define Vswp2(a,b) { double t_; \
    t_ = (a)[0], (a)[0] = (b)[0], (b)[0] = t_; \
    t_ = (a)[1], (a)[1] = (b)[1], (b)[1] = t_; \
}

double vect_det2(TDAContext *ctx, Vector v1,Vector v2)
{
    (void)ctx;        /* unused: the signature is shared */
    return ((v1[0] * v2[1]) - (v2[0] * v1[1]));
}

double vect_m_dist(TDAContext *ctx, Vector v1,Vector v2)
{
    (void)ctx;        /* unused: the signature is shared */
    double dx = v1[0] - v2[0];
    double dy = v1[1] - v2[1];
...
void RecursiveIsect(TDAContext *ctx, Vector p1,Vector p2,Vector q1,Vector q2,Vector ret);

int g_line_intersect(TDAContext *ctx, double p1x,double p1y,double p2x,double p2y, double q1x,double q1y,double q2x,double q2y, double *s1x,double *s1y,double *s2x,double *s2y,double eps)
{                  
    double rpx,rpy,rqx,rqy,t,deel;
    Vector p1,p2,q1,q2,s1,s2 = {0},p,pq1,pq2,q,qp1,qp2,s,qs;
    int c1,c2,c1s,c2s;
    ...
    if (dmax(ctx, p1[0],p2[0]) < dmin(ctx, q1[0],q2[0]) ||
        dmax(ctx, q1[0],q2[0]) < dmin(ctx, p1[0],p2[0]))
        return(0);
    ...
    if (fabs(deel) < eps) {           /* parallel */
        ...
        dc2 = vect_m_dist(ctx, p1,p2);
        d1 = vect_m_dist(ctx, p1,q1);
        d2 = vect_m_dist(ctx, p1,q2);
        ...
        *s2x = s2[0]; *s2y = s2[1];
        ...
        Vsub2(pq1,q1,p1);
        Vsub2(pq2,q2,p1);
        inp1 = vect_det2(ctx, p,pq1);
        inp2 = vect_det2(ctx, p,pq2);
        ...
        inp3 = vect_det2(ctx, q, qp1);
        inp4 = vect_det2(ctx, q, qp2);
        c2 = (inp3 * inp4 <= 0.0);

        {
            ...
            l_q = g_len(ctx, q1[0],q1[1],q2[0],q2[1]);
            l_p = g_len(ctx, p1[0],p1[1],p2[0],p2[1]);

            if (eps > g_len(ctx, s1[0],s1[1],p2[0],p2[1]) / l_p) {
                *s1 = *p2;
                c2 = 2;
            }
            else if (eps > g_len(ctx, s1[0],s1[1],p1[0],p1[1]) / l_p) {
                *s1 = *p1;
                c2 = 2;
            }
            else {
                if (eps > g_len(ctx, s1[0],s1[1],q2[0],q2[1]) / l_q) {
                    *s1 = *q2;
                    c1 = 2;
                }
                else if (eps > g_len(ctx, s1[0],s1[1],q1[0],q1[1]) / l_q) {
                    *s1 = *q1;
                    c1 = 2;
                }
            }
        ...
        Vsub2(s,s1,p1);
        inp1s = vect_det2(ctx, s,pq1);
        inp2s = vect_det2(ctx, s,pq2);
        c1s = Vequ2(s1, p1) ? -1 : inp1s * inp2s <= 0.0;

        Vsub2(qs,s1,q1);
        inp3s = vect_det2(ctx, qs,qp1);
        inp4s = vect_det2(ctx, qs,qp2);
        c2s = Vequ2(s1, q1) ? -1 : inp3s * inp4s <= 0.0;
        {
            ...
            if (!((c1 == 0) || (c2 == 0) || (c1s == (c1 != 0)) || (c1s == -1)))
                ...
                failed = 2;
            else if (c1 && c2 && (g_len(ctx, s1[0],s1[1],q1[0],q1[1]) > g_len(ctx, q2[0],q2[1],q1[0],q1[1])))
                failed = 3;
            else if (c1 && c2 && (g_len(ctx, s1[0],s1[1],q2[0],q2[1]) > g_len(ctx, q2[0],q2[1],q1[0],q1[1])))
                failed = 4;
            else if (c1 && c2 && (g_len(ctx, s1[0],s1[1],p1[0],p1[1]) > g_len(ctx, p2[0],p2[1],p1[0],p1[1])))
                failed = 5;
            else if (c1 && c2 && (g_len(ctx, s1[0],s1[1],p2[0],p2[1]) > g_len(ctx, p2[0],p2[1],p1[0],p1[1])))
                failed = 6;
            ...
            if (failed > 0)
                RecursiveIsect(ctx, p1,p2,q1,q2,s1);
...
void RecursiveIsect(TDAContext *ctx, Vector p1,Vector p2,Vector q1,Vector q2,Vector ret)
{
    ...
    if (g_vlen(ctx, q[0],q[1]) < g_len(ctx, p2[0],p2[1],p1[0],p1[1])) {
        RecursiveIsect(ctx, q1,q2,p1,p2,ret);
        return;
    }
    Vsub2(s1,p1,q1);
    Vsub2(s2,p2,q1);
...
/**
        assert(vect_det2(ctx, q, s1) <= 0);
        assert(vect_det2(ctx, q, s2) >= 0);
**/
        if (vect_det2(ctx, q,m) <= 0.0)
            Vcpy2(s1, m)
        else
            Vcpy2(s2, m)
