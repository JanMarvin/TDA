...
int qld(TDAContext *ctx, int n,int m,int meq,int lql,int maxit,double *a,double *b,double *grad, double *g,double *x,double *xl,double *xu,int *iact,int *nact, double vsmall,double *diag);
...
int qld(TDAContext *ctx, int n,int m,int meq,int lql,int maxit,double *a,double *b,double *grad, double *g,double *x,double *xl,double *xu,int *iact,int *nact, double vsmall,double *diag)
{
    ...
    int i,j = 0,k,k1,kk,id,ir,il,iu,ix,iy,iw,is,ju,nu;
    int iwz,iwr,iww,iwd,iwa,ifinc,kfinc,jfinc,iterc,itref,knext = 0,kflag;
    int iflag,lflag,jflag,kdrop,mflag,iwx,iwy,iws,iza,ira,irb;
    int ia,ii,jl,iwwn,iz,nflag,nm,lower,lw,info,mn; 
             
    double sum,suma,sumb,sumc,sumx,sumy = 0.0,cvmax,diagr,fdiffa,fdiff;
    double temp,tempa,step = 0.0,ratio,parinc = 0.0,parnew = 0.0,xmagr,xmag,ga,gb,res = 0.0;
    double *w; 
    double vfact = 1.0;

    info = 3;
    lw = (3 * n * n) / 2 + 10 * n + 2 * m + 10;
    if (!(w = (double *) calloc((size_t)(lw),sizeof(double))))  
        return(3);
    memrq(ctx, lw,sizeof(double));
...
/* IF NECESSARY INCREASE THE DIAGONAL ELEMENTS OF G. */
    ...
    *diag = 0.0;
    for (i = 1; i <= n; ++i) {
        id = iwd + i;
        w[id] = g[(i - 1) * n + i];           
        *diag = dmax(ctx, *diag,vsmall - w[id]);
        if (i == n)
            goto L60;
        ii = i + 1;
        for (j = ii; j <= n; ++j) {
            ga = -dmin(ctx, w[id],g[(j-1) * n + j]);
            gb = fabs(w[id]-g[(j - 1) * n + j]) + fabs(g[(i - 1) * n + j]);
            if (gb > 0.0)
                ga = ga + pow(g[(i - 1) * n + j],2.0) / gb;
            *diag = dmax(ctx, *diag,ga);
        }
L60:
        continue;
    }
...
L810:
    ira = ir;
    ir = ir + kdrop + 1;
    temp = dmax(ctx, fabs(w[ir - 1]),fabs(w[ir]));
...
L880:
    ...
    temp = dmax(ctx, fabs(w[is - 1]),fabs(w[is]));
