...
void balance(TDAContext *ctx, int n,double *a,double *d,int *low,int *hi);
void balbak(TDAContext *ctx, int n,double *a,double *d,int low,int hi);
void orthes(TDAContext *ctx, int n,double *a,int k,int l,double *d);
void ortrans(TDAContext *ctx, int n,double *h,double *d,int low,int upp,double *v);
int hqr2(TDAContext *ctx, int n,double *h,int low,int upp,double *vecs,double *wr,double *wi);
void cdiv(TDAContext *ctx, double xr,double xi,double yr,double yi,double *zr,double *zi);
...
void tridf1(TDAContext *ctx, int n, double *d, double *e, double *z);
int evectf(TDAContext *ctx, int n, double *d, double *e, double *z);
int simitz(TDAContext *ctx, int n,int p,int km,double eps,int em,double *x,double *d,int *nit);
double g_ranf(TDAContext *ctx);
double g_ip(TDAContext *ctx, int n,double *z,double *w);
void g_op(TDAContext *ctx, int n,double *z,double *w);
int g_imtql2(TDAContext *ctx, int nm,int n,double *d,double *e,double *z,int mxit);
void g_tred2(TDAContext *ctx, int nm,int n,double *a,double *d,double *e,double *z);
...
double scale(TDAContext *ctx, int n,double *a,double *veci,double *prfact);
double hesqr(TDAContext *ctx, int n,double *a,double *veci,double *evr,double *evi, double *subdia,int *indic,double ex);
int realve(TDAContext *ctx, int n,int m,int ivec,double *a,double *vecr,double *evr,double *evi, int *iwork,double *work,int *indic,double eps,double ex);
int compve(TDAContext *ctx, int n,int m,int ivec,double *a,double *vecr,double *veci,double *evr, double *evi,int *indic,int *iwork,double *subdia,double *work1, double *work2,double *work,double eps,double ex);
void prxxx(TDAContext *ctx, char *txt,int n,int m,double *x);
...
int eigen(TDAContext *ctx, int n,double *a,double *evr,double *evi,double *vec,int bal)
{
    ...
    if (bal)
        balance(ctx, n,a,d,&low,&hi);

    orthes(ctx, n,a,low,hi,d);   /* reduction to Hessenberg form */

    ortrans(ctx, n,a,d,low,hi,vec);  /* calculate transformation matrix */

    r = hqr2(ctx, n,a,low,hi,vec,evr,evi);   /* calc eigenvalues and vectors */

    if (bal)                        /* back transformation */
        balbak(ctx, n,a,d,low,hi);

    /* normalization of eigenvectors */

    i = 1;
    while (i <= n) {
        if (fabs(evi[i]) < ctx->EPSI) {    /* if eigenvalue real */
            tmp = 0.0;
            for (j = 1; j <= n; ++j) {
                tmp1 = vec[(j - 1) * n + i];
                volatile double p = tmp1 * tmp1;
                tmp += p;
            }
            if (tmp > 0.0) {
                tmp = sqrt(tmp);
            ...
            for (j = 1; j <= n; ++j) {
                ...
                vr = vec[jn];
                vi = vec[jn + 1];
                volatile double p1 = vr * vr;
                volatile double p2 = vi * vi;
                tmp = p1 + p2;
                if (vri < tmp) {
                    vri = tmp;
                    jj = j;
            ...
            if (vri > 0.0) {
                ...
                for (j = 1; j <= n; ++j) {
                    ...
                    tmp = vec[jn];
                    tmp1 = vec[jn + 1];
                    volatile double p1 = tmp * vr;
                    volatile double p2 = tmp1 * vi;
                    vec[jn] = p1 + p2;

                    volatile double p3 = tmp1 * vr;
                    volatile double p4 = tmp * vi;
                    vec[jn + 1] = p3 - p4;
                }
            }
            i += 2;
...
void orthes(TDAContext *ctx, int n,double *a,int k,int l,double *d)
{
    ...
    tol = ctx->DBLMIN / ctx->EPSI;  /* DBLMIN smallest number representable   */
...
int hqr2(TDAContext *ctx, int n,double *h,int low,int upp,double *vecs,double *wr,double *wi)
{
    register int i,j,k,l;
    int m,en,na,its;
    double norm,p,q,r = 0.0,ra,s = 0.0,sa,t,vr,vi,w,x,y,z = 0.0,tmp1,tmp2;
        ...
        if (q == 0.0) {
            ...
            if (q < 0.0) {
                ...
                if (fabs(h[(en - 1) * n + na]) > fabs(h[(na - 1) * n + en])) {
                    ...
                    cdiv(ctx, -h[(na - 1) * n + en],0.0,h[(na - 1) * n + na] - p,q,
                ...
                for (i = na - 1; i >= 1; --i) {
                    ...
                    if (wi[i] < 0.0) {
                        ...
                        if (wi[i] == 0.0) {
                            ...
                            cdiv(ctx, x * r - z * ra + q * sa,x * s - z * sa - q * ra,
...
int svdecomp(TDAContext *ctx, int m,int n,double *a,double *q,double *u,double *v,int mode)
{
    ...
    double c,f = 0.0,f1,g,h,s,x,y,z,eps,tol;
    double *e;
    ...
    *********************************************************************/
    ...
    eps = 2.0 * ctx->EPSI;
    tol = ctx->DBLMIN / ctx->EPSI;

    if (!(e = (double *) calloc ((size_t)(n + 1),sizeof(double))))
        return(-1);
    memrq(ctx, n + 1,sizeof(double));
...
int ginv(TDAContext *ctx, int m,int n,double *a)
{
    ...
    if (!(q = (double *) calloc ((size_t)(n + 1),sizeof(double)))) {
        return(-1);
    }
    if (!(u = (double *) calloc ((size_t)(m * n + 1),sizeof(double)))) {
        free((char *)q);
        return(-1);
    }
    if (!(v = (double *) calloc ((size_t)(n * n + 1),sizeof(double)))) {
        free((char *)q);
        free((char *)u);
        return(-1);
    }
    memrq(ctx, n * (1 + n + m) + 3,sizeof(double));

    if ((r = svdecomp(ctx, m,n,a,q,u,v,3)) < 0)
        goto GINV_Fin;

    r = 0;
    for (i = 1; i <= n; ++i) {
        if (fabs(q[i]) > ctx->EPSI1)
            r++;
        for (j = 1; j <= m; ++j) {
            tmp = 0.0;
            for (k = 1; k <= n; ++k) {
                if (fabs(q[k]) > ctx->EPSI1)
...
int ginv1(TDAContext *ctx, int m,int n,double *a,double *sv)
{
    ...
    memrq(ctx, n * (n + m) + 2,sizeof(double));

    if ((r = svdecomp(ctx, m,n,a,sv,u,v,3)) < 0)
        goto GINV1Fin;
...
int simitz(TDAContext *ctx, int n,int p,int km,double eps,int em,double *x,double *d,int *nit)
{
    ...
    int ik,jk,g,ig,h,z1,z2,ks,m,m1 = 0,lf,l1,jp,orig,gflag;
    int err,cxa,fa,rqa,ua,ba,mxit;
    double e,e1 = 0.0,e2 = 0.0,s,t,tol;
    ...
    */
    ...
    if (!(cx = (double *) calloc ((size_t)(p + 1),sizeof(double))))
        goto SIMITZFin;
    cxa = p + 1;
    memrq(ctx, cxa,sizeof(double));

    if (!(f = (double *) calloc ((size_t)(p + 1),sizeof(double))))
        goto SIMITZFin;
    fa = p + 1;
    memrq(ctx, fa,sizeof(double));

    if (!(rq = (double *) calloc ((size_t)(p + 1),sizeof(double))))
        goto SIMITZFin;
    rqa = p + 1;
    memrq(ctx, rqa,sizeof(double));

    if (!(u = (double *) calloc ((size_t)(2 * n + 1),sizeof(double))))
        goto SIMITZFin;
    ua = 2 * n + 1;
    memrq(ctx, ua,sizeof(double));

    if (!(b = (double *) calloc ((size_t)(p * p + 1),sizeof(double))))
        goto SIMITZFin;
    ba = p * p + 1;
...
L150:
    ...
    g_tred2(ctx, p,p - g,b + (ig-1)*p + ig-1,d + ig-1,u,b + (ig-1)*p + ig-1);
    l = g_imtql2(ctx, p,p - g,d + ig-1,u,b + (ig-1)*p + ig-1,mxit);

    f[ig] = dmax(ctx, f[ig],1000.0 * (double)l);

    for (k = ig; k <= p; ++k)
        d[k] = sqrt(dmax(ctx, -d[k],0.0));
...
L1140:
    for (k = 1; k <= jp; ++k) {
        g_op(ctx, n,x + (k - 1) * n,x + (p - 1) * n);
        for (i = 1; i <= k; ++i)
            b[(i - 1) * p + k] = -g_ip(ctx, n,x + (i - 1) * n,x + (p - 1) * n);
    }
    g_tred2(ctx, p,jp,b,d,u,b);
    l = g_imtql2(ctx, p,jp,d,u,b,mxit);
...
double g_ranf(TDAContext *ctx)
{
    ...
    if (ctx->s_g_ranf_init == 0) {
        if (ctx->s_g_ranf_seed > 1.0)
            ctx->s_g_ranf_seed = 1.0 / ctx->s_g_ranf_seed;
        ctx->s_g_ranf_k = 2 * (int)(16384.0 * ctx->s_g_ranf_seed) - 1;
        ctx->s_g_ranf_init = 1;
    }
    ctx->s_g_ranf_k = (3125 * ctx->s_g_ranf_k) % 65536;
    return(fabs((double)(ctx->s_g_ranf_k - 32768) / 32768.0));
}
...
L105:
        for (m = l; m <= n; ++m) {
            if (m == n)
                break;
            if (fabs(e[m]) <= ctx->EPSI * (fabs(d[m]) + fabs(d[m + 1])))
                break;
        }
        p = d[l];
        if (m == l)
...
Fin:                            /* Uebertragen der Eigenwerte aus a in  */
    ...
    while (++i <= n) {
        tmp = a[(i - 1) * n + i];
        /* the superdiagonal entry exists only for i < n; loading it
           unconditionally read one element past the matrix on the
           last row */
        tmp1 = (i < n) ? a[(i - 1) * n + i + 1] : 0.0;
...
int eigen1(TDAContext *ctx, int n,double *a,double *evr,double *evi, double *vecr,double *veci,int *indic)
{
    ...
    if (!(iwork = (int *)calloc((size_t)(n + 1),sizeof(int))))
        goto fin;
    memrq(ctx, n + 1,sizeof(int));
    iworka = n + 1;

    if (!(local = (int *)calloc((size_t)(n + 1),sizeof(int))))
        goto fin;
    memrq(ctx, n + 1,sizeof(int));
    locala = n + 1;

    if (!(prfact = (double *)calloc((size_t)(n + 1),sizeof(double))))
        goto fin;
    memrq(ctx, n + 1,sizeof(double));
    prfacta = n + 1;

    if (!(subdia = (double *)calloc((size_t)(n + 1),sizeof(double))))
        goto fin;
    memrq(ctx, n + 1,sizeof(double));
    subdiaa = n + 1;

    if (!(work = (double *)calloc((size_t)(n + 1),sizeof(double))))
        goto fin;
    memrq(ctx, n + 1,sizeof(double));
    worka = n + 1;

    if (!(work1 = (double *)calloc((size_t)(n + 1),sizeof(double))))
        goto fin;
    memrq(ctx, n + 1,sizeof(double));
    work1a = n + 1;

    if (!(work2 = (double *)calloc((size_t)(n + 1),sizeof(double))))
        goto fin;
    memrq(ctx, n + 1,sizeof(double));
    work2a = n + 1;
    ...
    enorm = scale(ctx, n,a,veci,prfact);
    ...
    ****/
    ...
    eps = hesqr(ctx, n,a,veci,evr,evi,subdia,indic,ex);
    ...
    for (i = 1; i <= n; ++i) {
        ...
        if (indic[ivec] != 0) {
            if (evi[ivec] == 0.0) {
                ...
                realve(ctx, n,m,ivec,a,vecr,evr,evi,iwork,work,indic,eps,ex);
            }
            else {
            ...
            conjugate. */
                ...
                if (kon != 0)
                    ...
                    kon = 1;
                    compve(ctx, n,m,ivec,a,vecr,veci,evr,evi,indic,iwork,subdia,
    ...
    if (n > 2) {
        ...
        for (k = 1; k <= m; ++k) {
            ...
            for (j = 2; j <= n; ++j) {
                ...
                for (i = l; i <= n; ++i) {
                   d2 = veci[(i - 1) * n + k];
                   volatile double prod1 = d2 * a[(j - 1) * n + i];
                   d1 += prod1;
                }
                for (i = l; i <= n; ++i) {
                    volatile double prod2 = veci[(i - 1) * n + k] * d1;
                    a[(j - 1) * n + i] -= prod2;
                }
            }
        }
...
lab1016:
        for (j = 1; j <= n; ++j) {
            ...
            for (k = 1; k <= n; ++k) {
                d3 = a[(j - 1) * n + k];
                volatile double p1 = d3 * vecr[(k - 1) * n + i];
                d1 += p1;
                if (l != 0) {
                    volatile double p2 = d3 * vecr[(k - 1) * n + i - 1];
                    d2 += p2;
                }
            }
            if (prfact[j] == 0.0) {
        ...
        d1 = 0.0;
        for (m = 1; m <= n; ++m) {
            volatile double p = work[m] * work[m];
            d1 += p;
        }
...
lab1021:
        ...
        r = 0.0;
        for (j = 1; j <= n; ++j) {
            volatile double p1 = work[j] * work[j];
            volatile double p2 = subdia[j] * subdia[j];
            r1 = p1 + p2;
            if (r < r1) {
                r = r1; l = j;
            }
        ...
        for (j = 1; j <= n; ++j) {
            ...
            volatile double p1 = d1 * d3;
            volatile double p2 = d2 * r1;
            vecr[(j - 1) * n + i] = (p1 + p2) / r;

            volatile double p3 = d2 * d3;
            volatile double p4 = d1 * r1;
            veci[(j - 1) * n + i] = (p3 - p4) / r;
...
fin:
    if (iworka > 0) {
        free((char *)iwork);
        memrq(ctx, -iworka,sizeof(int));
    }
    if (locala > 0) {
        free((char *)local);
        memrq(ctx, -locala,sizeof(int));
    }
    if (prfacta > 0) {
        free((char *)prfact);
        memrq(ctx, -prfacta,sizeof(double));
    }
    if (subdiaa > 0) {
        free((char *)subdia);
        memrq(ctx, -subdiaa,sizeof(double));
    }
    if (worka > 0) {
        free((char *)work);
        memrq(ctx, -worka,sizeof(double));
    }
    if (work1a > 0) {
        free((char *)work1);
        memrq(ctx, -work1a,sizeof(double));
    }
    if (work2a > 0) {
        free((char *)work2);
        memrq(ctx, -work2a,sizeof(double));
    }
    if (err)
        return(err);
...
{
    int i,iter,j,ncount;
    double enorm,bound1,bound2,column,row,factor,fnorm,q;
