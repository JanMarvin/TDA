...
int mean(TDAContext *ctx, int n,short *vinum,double *m);
int std(TDAContext *ctx, int n,short *vinum,double *m,double *s,int opt);
int corr(TDAContext *ctx, int n,short *vinum,double *m,double *s,double *x);
...
int mean(TDAContext *ctx, int n,short *vinum,double *m)
{
    ...
    for (i = 0; i < ctx->NOC; ++i) {
        ...
        for (j = 0; j < n; ++j) {
            if (ctx->WIVar >= 0)
                m[j] += get_data(ctx, vinum[j],i) * wt;
            else
                m[j] += get_data(ctx, vinum[j],i);
        }
    }
    for (j = 0; j < n; ++j) {
        if (wsum > 0.0)
...
int std(TDAContext *ctx, int n,short *vinum,double *m,double *s,int opt)
{
    ...
    for (i = 0; i < ctx->NOC; ++i) {
        ...
        for (j = 0; j < n; ++j) {
            tmp = get_data(ctx, vinum[j],i) - m[j];
            tmp *= tmp;
            if (ctx->WIVar >= 0)
                s[j] += tmp * wt;
            else
...
void rcorr(TDAContext *ctx, int n,short *vinum,double *x)
{
    ...
    for (j = 0; j < n; ++j) {
        for (k = 0; k <= j; ++k) {
            ...
            xy = 0.0;
            for (ii = 1; ii < ctx->NOC; ++ii) {

                xi = get_data(ctx, vinum[j],ii);            
                yi = get_data(ctx, vinum[k],ii);            

                for (jj = 0; jj < ii; ++jj) {

                    xj = get_data(ctx, vinum[j],jj);            
                    yj = get_data(ctx, vinum[k],jj);            

                    xy += dsign(ctx, xi - xj) * dsign(ctx, yi - yj);
                }
            }
            x[l] = xy / nn;
            l++;
