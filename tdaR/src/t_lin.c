...
void dqrdc(TDAContext *ctx, double *x,int cdx,int n,int p,double *qraux,int *jpvt,       double *work,int job);
void dqrsl(TDAContext *ctx, double *x,int cdx,int n,int k,double *qraux,double *y,double *qy, double *qty,double *b,double *rsd,double *xb,int job,int *info);
...
L70:    /* compute inverse(r) */
    ...
    for (k = 1; k <= n; ++k) {
        ...
        dscal(ctx, k - 1,t,a + k - 1,ma);

        if (k + 1 <= n) {        
            for (j = k + 1; j <= n; ++j) {
                t = a[(k - 1) * ma + j];
                a[(k - 1) * ma + j] = 0.0;   
                daxpy(ctx, k,t,a + k - 1,ma,a + j - 1,ma);
            }
        }
    }
    ...
    for (j = 1; j <= n; ++j) {
        if (j > 1) {             
            for (k = 1; k < j; ++k) {
                t = a[(k - 1) * ma + j];
                daxpy(ctx, k,t,a + j - 1,ma,a + k - 1,ma);
            }
        }
        t = a[(j - 1) * ma + j];
        dscal(ctx, j,t,a + j - 1,ma);
    }
    return(0);
}           
...
int dpofa(TDAContext *ctx, double *a,int ma,int n)
{
    ...
    for (j = 1; j <= n; ++j) {
        ...
        if (j > 1) {
            for (k = 1; k < j; ++k) {
                kk = (k - 1) * ma + k;
                t = a[(k - 1) * ma + j] - ddot(ctx, k - 1,a + k - 1,ma,a + j - 1,ma);
                if (a[kk] == 0.0)
                    return(-1);
                t /= a[kk];
                a[(k - 1) * ma + j] = t;
...
void dqrdc(TDAContext *ctx, double *x,int cdx,int n,int p,double *qraux,int *jpvt,       double *work,int job)
{
    ...
    for (j = 1; j <= p; ++j) {
        ...
        if (swapj) {           
            if (j != pl)
                dswap(ctx, n,x+pl-1,cdx,x+j-1,cdx);
            jpvt[j] = jpvt[pl];
            jpvt[pl] = j;
            pl++;       
        }             
    ...
    for (jj = 1; jj <= p; ++jj) {
        ...
        if (jpvt[j] < 0) {         
            jpvt[j] = -jpvt[j];
            if (j != pu) {              
                dswap(ctx, n,x+pu-1,cdx,x+j-1,cdx);
                jp = jpvt[pu];
                jpvt[pu] = jpvt[j];
                jpvt[j] = jp;
            }
...
L60:   /* COMPUTE THE NORMS OF THE FREE COLUMNS. */
    ...
    for (l = 1; l <= lup; ++l) {
        ...
        if (maxj != l) {           
            dswap(ctx, n,x+l-1,cdx,x+maxj-1,cdx);
            qraux[maxj] = qraux[l];
            work[maxj] = work[l];
            jp = jpvt[maxj];
            jpvt[maxj] = jpvt[l];
...
L120:
        ...
        nrmxl = dnrm2(ctx, n-l+1,x+(l-1)*cdx+l-1,cdx);
        if (nrmxl == 0.0)
            continue; 

        if (x[(l-1)*cdx+l] != 0.0)
            nrmxl = fsign(ctx, nrmxl,x[(l-1)*cdx + l]);

        dscal(ctx, n-l+1,1.0/nrmxl,x+(l-1)*cdx+l-1,cdx);
        x[(l-1)*cdx+l] += 1.0;
        ...
        if (p >= lp1) {           
            for (j = lp1; j <= p; ++j) {
                t = -ddot(ctx, n-l+1,x+(l-1)*cdx+l-1,cdx,x+(l-1)*cdx+j-1,cdx)/
                                                                x[(l-1)*cdx+l];                

                daxpy(ctx, n-l+1,t,x+(l-1)*cdx+l-1,cdx,x+(l-1)*cdx+j-1,cdx);
                if (j < pl || j > pu)
                    goto L150;
                if (qraux[j] != 0.0) {          
                    ...
                    if (tt != 1.0)
                        ...
                        qraux[j] = dnrm2(ctx, n-l,x+l*cdx+j-1,cdx);
                        work[j] = qraux[j];
                    }
                }
L150:           ;          
...
void dqrsl(TDAContext *ctx, double *x,int cdx,int n,int k,double *qraux,double *y,double *qy, double *qty,double *b,double *rsd,double *xb,int job,int *info)
{
    ...
    *info = cqy = cqty = cb = cr = cxb = 0;
    ...
    if (cqy)
        dcopy(ctx, n,y,1,qy,1);
    if (cqty)
        dcopy(ctx, n,y,1,qty,1);
    if (!cqy)
        goto L70;
    ...
    for (jj = 1; jj <= ju; ++jj) {
        ...
        temp = x[(j-1)*cdx+j];
        x[(j-1)*cdx+j] = qraux[j];
        t = -ddot(ctx, n-j+1,x+(j-1)*cdx+j-1,cdx,qy+j-1,1) / x[(j-1)*cdx+j];
        daxpy(ctx, n-j+1,t,x+(j-1)*cdx+j-1,cdx,qy+j-1,1);
        x[(j-1)*cdx+j] = temp;
    }
...
L70:
    ...
    for (j = 1; j <= ju; ++j) {
        ...
        temp = x[(j-1)*cdx+j];
        x[(j-1)*cdx+j] = qraux[j];
        t = -ddot(ctx, n-j+1,x+(j-1)*cdx+j-1,cdx,qty+j-1,1) / x[(j-1)*cdx+j];
        daxpy(ctx, n-j+1,t,x+(j-1)*cdx+j-1,cdx,qty+j-1,1);
        x[(j-1)*cdx+j] = temp;
    }
...
L100:  /* SET UP TO COMPUTE B, RSD, OR XB. */
    ...
    if (cb)
        dcopy(ctx, k,qty,1,b,1);
    kp1 = k + 1;
    if (cxb)
        dcopy(ctx, k,qty,1,xb,1);
    if (cr && k < n)
        dcopy(ctx, n-k,qty+kp1-1,1,rsd+kp1-1,1);
    ...
    if (cr || cxb) {    /* COMPUTE RSD OR XB AS REQUIRED. */
        ...
        for (jj = 1; jj <= ju; ++jj) {
            ...
            if (qraux[j] != 0.0) {           
                ...
                x[(j-1)*cdx+j] = qraux[j];
                if (cr) {              
                    t = -ddot(ctx, n-j+1,x+(j-1)*cdx+j-1,cdx,rsd+j-1,1) / x[(j-1)*cdx+j];
                    daxpy(ctx, n-j+1,t,x+(j-1)*cdx+j-1,1,rsd+j-1,1);
                }
                if (cxb) {             
                    t = -ddot(ctx, n-j+1,x+(j-1)*cdx+j-1,cdx,xb+j-1,1) / x[(j-1)*cdx+j];
                    daxpy(ctx, n-j+1,t,x+(j-1)*cdx+j-1,cdx,xb+j-1,1);
                }
                x[(j-1)*cdx+j] = temp;
            }
        }
...
void dtrco(TDAContext *ctx, double *t,int cdt,int n,double *rcond,double *z,int job)
{
    ...
    for (j = 1; j <= n; ++j) {
        ...
        if (lower)
            i1 = j;
        tnorm = dmax(ctx, tnorm,dasum(ctx, l,t+(i1-1)*cdt+j-1,cdt));
    }
    ...
    for (kk = 1; kk <= n; ++kk) {
        ...
        if (kk < n) {          
            w = -z[k];
            daxpy(ctx, n-kk,w,t+(i1-1)*cdt+k-1,cdt,z+i1-1,1);
        }
    }
...
void dtrsl(TDAContext *ctx, double *t,int cdt,int n,double *b,int job,int *info)                                                 
{
    ...
    *info = 0;
    ...
    for (j = 2; j <= n; ++j) {
        temp = -b[j-1];
        daxpy(ctx, n-j+1,temp,t+(j-1)*cdt+j-2,cdt,b+j-1,1);
        b[j] = b[j]/t[(j-1)*cdt+j];        
    }
    return;
...
L80:
    ...
    for (jj = 2; jj <= n; ++jj) {
        j = n - jj + 1;
        b[j] -= ddot(ctx, jj-1,t+j*cdt+j-1,cdt,b+j,1);
        b[j] = b[j]/t[(j-1)*cdt+j];
    }
    return;   
...
L110:
    b[1] = b[1]/t[1];  
    for (j = 2; j <= n; ++j) {
        b[j] -= ddot(ctx, j-1,t+j-1,cdt,b,1);
        b[j] = b[j]/t[(j-1)*cdt+j];
    }
}           
