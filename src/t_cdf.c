...
int mecdf(int ndim,double *d,double *rho,double *prob);
double xphi(double x,double y);
int dmv(int m,int k,double *h,double *r,double *prob,double eps,double *err);
int dmv1(int m,int k,double *h,double *r,double *prob);
int dmvsolve(int mr,int m,int k,double *h,double *r,double *prob);
double bivn(double ah,double ak,double r,int *err);

/* ------------------------------------------------------------------------ */
/*  Global variables                                                        */

double DMVArg[DMVMax+1];        /* arguments for dmv()                      */
...
double cdnif(double p)
{
    ...
    if (p <= 0.0 || p >= 1.0) { 
        printfe("Error in cdnif with argument %g\n",p);
        gerr_exit(27);
    }  
    q = p - 0.5;
    if (fabs(q) <= split) {
...
double cdnif1(double p)
{
    ...
    if (p > 0.5)
        ...
        x = p;
    if (x < 0.0) {
        printfe("Error in cdnif1 with argument %g\n",p);
        gerr_exit(28);
    }  
    s = sqrt(-2.0 * log(x));
...
double cdtf(double t, int df)
{
    ...
    if (df < 1) {
        printfe("Error in cdtf: %d degrees of freedom not possible.\n",df);
        gerr_exit(29);
    }
    t = fabs(t);
    t1 = t / sqrt((double)df);
...
double cdnm(double x)
{
    ...
    if (x == 0.0) 
        ...
        if (x <= 2.0) {
            ...
            b2 = a1 + 1.0;
            t  = a2 / b2;
            while (fabs(r-t) > EPSI2 && fabs(s-t) > EPSI2) {
                a += 1.0;
                a0 = a1;
                a1 = a2;
                a2 = x * a1 + a * a0;
...
int mecdf(int ndim,double *d,double *rho,double *prob)
{
    ...
    *prob = 1.0 - cdnf(z[1][0]);           
    ...
    for (i = 2; i <= ndim; ++i) {
        ...
        probi = 1.0 - cdnf(z[i][i - 1]);
        if (probi <= 0.0)           
            return(i);
                  
        *prob *= probi;         
        if (i < ndim) {       
            u[i] = xphi(z[i][i - 1],0.0) / probi;
...
double SQ2P = 0.91893853320467274;   
