...
#include <string.h>
#include <winsock.h>
#include "t_xnt.h"        /* Includes X.h, Xlib.h, Xutil.h mainly. */
...
NT_window *window;
{
	...
	if (window->frame != NULL )
	{
		...
		{
	    frame_style = mwmHintsToFrameStyle((MwmHints *)hints);
		}
		else
	{
...
XLoadQueryFont(Display *display, const char *name)
{
	...
	cjh_rel_dc(root,hdc);
	fs->min_bounds.width = tm.tmMaxCharWidth;
	...
	for (i=0; i<256; i++) {
		fs->per_char[i].width = tm.tmMaxCharWidth;
		fs->per_char[i].rbearing = tm.tmMaxCharWidth;
	}
