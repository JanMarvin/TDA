...
int lnsrch(int n,double *x,double f,double *g,double *p,double *xpls,
    double *fpls,int *mxtake,double stepmx,double *typx,
    double *w2);
void zhz(int nc,int n,double *y,double *h,double *u,double *t);
void solvew(int nc,int n,double *al,double *u,double *w,double *b);
void dstar(int nc,int n,double *u,double *s,double *w1,double *w2,double *w3,
    double sigma,double *al,double *d);
void mkmdl(int nc,int n,double f,double fp,double *g,double *gp,double *s,
    double *h,double *alpha,double *beta,double *sh,double *a);
void sigma(double *sgstar,double a,double b,double c,double d);
void roots(double *s1,double *s2,double *s3,double a,double b,double c,double d);
void sortrt(double *s1,double *s2,double *s3);
int fstofd(int nc,int m,int n,double *xpls,double *fpls,double *a,double *typx,
    double rnoise,double *fhat,int icase);
int sndofd(int nc,int n,double *xpls,double fpls,double *a,double *typx,
    double rnoise,double *stepsz,double *anbr);
void bakslv(int nc,int n,double *a,double *x,double *b);
void forslv(int nc,int n,double *a,double *x,double *b);
void choldr(int nc,int n,double *h,double *g,int *pivot,double *e,
    double *diag,double addmax);
void modchl(int ndim,int n,double *a,double *g,double tau1,
    double tau2,int *p,double *e);
void ts_init(int n,int ndim,double *a,int *phase1,double *delta,int *p,double *g,
    double *e,double *ming,double tau1,double *gamma,double *taugam);
void gersch(int ndim,int n,double *a,int j,double *g);
void fin2x2(int ndim,int n,double *a,double *e,double tau2,double *delta,
    double gamma);
void slvmdl(int nc,int n,double *h,double *u,double *t,double *e,double *diag,
    double *s,double *g,int *pivot,double *w1,double *w2,double *w3,
    double alpha,double beta,int *nomin);            
int optstp(int n,double *xpls,double fpls,double *gpls,double *x,           
    int *icscmx,double fscale,
    int mxtake,double *rgx,double *rsx,int iretcd);
int ts_func(int n,double *x,double *f,int gflg,double *g,int hflg,double *h);
void ts_prn(int iter,double f,double gnorm,double pc,int n,double *x,double *g);
void ts_prn1(int n,double *h);

/* ------------------------------------------------------------------------ */
/*  global variables                                                        */

int TSMETH = 0;     /* 0 if quadratic (Newton) model                        */
                    /* 1 if tensor model                                    */

int TSDERIV = 0;    /* 0 if only function values                            */
                    /* 1 if also analytical gradient                        */
                    /* 2 if also analytical hessian                         */

int TSFNC = 0;      /* 0 use standard models, 1 user-defined functions      */
int TSMOD = 0;      /* model number for TSFNC = 0.                          */
    ...
    double *h,int *pivot) 
{
    ...
    double f,finit,fn,fp,fscale,temp,fpls,rnf,analtl,gnorm;
    double addmax,tmp,rgx,rsx,alpha,beta,gd,stepmx;

    /* initialization */

    Iter = 0;                   /* iteration counter */
    NumF = NumFG = NumFH = 0;   /* function calls */

    iretcd = 2;                 /* for initial call of optstp() */
    ...
    if (TSDERIV == 1)     /* if analytical gradient */
        gflg = 1;

    if (TSDERIV == 2)     /* if analytical hessian */
        hflg = gflg = 1;
    ...
    *************************/
    ...
    if (stepmx <= 0.0) {             
        ...
        tmp = sqrt(tmp);
        stepmx = dmax(1000.0 * tmp,1000.0);
    }
    /****
    printf1("stepmx=%16.8e\n",stepmx);
    ****/
    ...
    rnf = dmax(pow(10.0,-tmp),EPSI);
    printf1("rnf=...%lg\n",rnf);
    *******************************/

    rnf = dmax(pow(10.0,-PMNDIGIT),EPSI);                      
    analtl = dmax(0.01,sqrt(rnf));
    ...
    err = ts_func(n,wk1,&f,gflg,g,hflg,h);
    if (err)
        return(-1);

    if (gflg == 0) {    /* finite difference gradient */

        err = fstofd(1,1,n,wk1,&f - 1,g,typx,rnf,wk2,1);
        if (err)
            return(-1);
    } 
    gnorm = twonrm(n,g);

    ts_prn(Iter,f,gnorm,0.0,n,wk1,g);
    ...
    if (gnorm <= TOLSG) {       
        ...
        itrmcd = optstp(n,xpls,fpls,gpls,x,&icscmx,fscale,
    ...
    if (hflg == 0) {         
        /**********     not used
        if (gflg)           
            fstofd(n,n,n,wk1,g,h,typx,rnf,wk2,3);                   
        else
        ***********/
        err = sndofd(n,n,wk1,f,h,typx,rnf,wk2,d);
        if (err)
            return(-1); 
    }
    ts_prn1(n,h);
    ...
    for (i = 2; i <= n; ++i)  
        dcopy(i - 1,h + (i - 1) * n,1,h + i - 1,n);

    /* CHOLESKY DECOMPOSITION FOR H (H=LLT) */
   
    choldr(n,n,h,d,pivot,e,wk1,addmax);
    ...
    forslv(n,n,h,wk1,wk2);
    bakslv(n,n,h,d,wk1);
 
    /* APPLY LINESEARCH TO THE NEWTON STEP */
              
    iretcd = lnsrch(n,x,f,g,d,xpls,&fpls,&mxtake,stepmx,typx,wk1);
    if (iretcd < 0)
        return(-1);
    ...
    if (gflg)                                       /* compute gradient */
        err = ts_func(n,wk1,&tmp,1,gpls,hflg,h);
    else  
        err = fstofd(1,1,n,wk1,&fpls - 1,gpls,typx,rnf,wk2,1);         
    ...
    ts_prn(Iter,fpls,rgx,rsx,n,x,gpls);
...
L200:
    ...
    if (TSMETH == 0) { 
        ...
        choldr(n,n,h,wk2,pivot,e,wk1,addmax);
    ...
    mkmdl(n,n,f,fp,gpls,g,s,h,&alpha,&beta,wk1,d);
    ...
    slvmdl(n,n,h,xpls,wk2,e,g,s,gpls,pivot,d,wk1,dn,alpha,beta,&nomin);
...
L300:
    ...
    if (nomin == 0) {      
        ...
        iretcd = lnsrch(n,x,f,g,dn,wk2,&fn,&mxtake,stepmx,typx,wk1);
        if (iretcd < 0)
            return(-1);
    ...
    if (gflg)  
        err = ts_func(n,wk1,&fpls,1,gpls,hflg,h);
    else
        err = fstofd(1,1,n,wk1,&fpls - 1,gpls,typx,rnf,wk2,1);                     
...
L350:
    ...
    FMin = fpls;        /* final function value */
    ...
    if (hflg == 0) { 
        /*************** not used
        if (gflg)           
            fstofd(n,n,n,x,gpls,h,typx,rnf,wk2,3);                   
        else
        *****************/
        err = sndofd(n,n,x,fpls,h,typx,rnf,wk2,d);
        if (err)
            return(-1);
    }
    ts_prn1(n,h);
...
L10:
    ...
    for (i = 1; i <= n; ++i) {
        temp = 1.0;
        temp1 = fabs(x[i]);
        temp2 = dmax(temp1,temp);
        temp1 = fabs(p[i]);
        rln = dmax(rln,temp1 / temp2);
    }
    almbmn = TOLSP / rln;                                                
    almbda = 1.0;                                                       
...
L100:
    ...
    err = ts_func(n,w2,fpls,0,&ttmp,0,&ttmp);
    if (err)
        return(-1); 
...
{
    register int i,j;
    double ynorm,d,s,temp1,temp2,sgn;
...
{    
    ...
    for (j = 1; j <= n; ++j) {
        xtmpj = xpls[j];                                                  
        stepsz = sqrt(rnoise) * dmax(fabs(xpls[j]),1.0);                       
        xpls[j] = xtmpj + stepsz;                                           

        err = ts_func(n,xpls,&f,0,&tmp,0,&tmp);
        if (err)
            return(-1);
...
{    
    ...
    for (i = 1; i <= n; ++i) {
        xtmpi = xpls[i];                                                  
        stepsz[i] = pow(rnoise,ov3) * dmax(fabs(xpls[i]),1.0);                  
        xpls[i] = xtmpi + stepsz[i];                                        

        err = ts_func(n,xpls,anbr + i,0,&tmp,0,&tmp);
        if (err)
            return(-1);
    ...
    for (i = 1; i <= n; ++i) {
        ...
        err = ts_func(n,xpls,&fhat,0,&tmp,0,&tmp);
        if (err)
            return(-1);
    ...
    double *diag,double addmax)
{
    ...
    tau2 = tau1= pow(EPSI,1.0 / 3.0);

    modchl(nc,n,h,g,tau1,tau2,pivot,e);
    ...
    for (j = 1; j < n; ++j) {
        ...
        if (phase1 == 0) {         
            ...
            if (j != n - 1) {       
                ...
                temp = dmax(normj,taugam);
                delta1 = temp - a[(j - 1) * ndim + j];
                temp = 0.0;
                delta1 = dmax(temp,delta1);
                delta =  dmax(delta1,delta);    
                e[j] = delta;
                a[(j - 1) * ndim + j] += e[j];
...
{                                                                            
    ...
    lmbd1 = (t1 - t3) / 2.0;
    lmbd2 = (t1 + t3) / 2.0;
    lmbdhi = dmax(lmbd1,lmbd2);
    lmbdlo = dmin(lmbd1,lmbd2);
    ...
    delta1 = dmax(delta1,gamma);
    delta1 = tau2 * delta1 - lmbdlo;
    temp = 0.0;
    *delta = dmax(*delta,temp);
    *delta = dmax(*delta,delta1);
...
{
    register int i,j;
    double shs,w11,w12,w13,w22,w23,w33,ca,cb,cc,sg,cd,r,r1,r2,temp,uu,ss;
    ...
    *nomin = 0;       
    ...
    if (n > 1) {       
        ...
        choldr(nc,n - 1,h,t,pivot,e,diag,addmax);
    }           
    ...
    if (n > 1) {       
        ...
        solvew(nc,n,h,u,w1,t);
        solvew(nc,n,h,u,w2,t);
        solvew(nc,n,h,u,w3,t);
    }
    ...
    dstar(nc,n,u,s,w1,w2,w3,sgstar,h,w1);
...
L200:
    ...
    if (n == 1) {       
        choldr(nc,n,h,t,pivot,e,diag,addmax);
    }
    else {
...
int optstp(int n,double *xpls,double fpls,double *gpls,double *x,           
    int *icscmx,double fscale,int mxtake,double *rgx,double *rsx,int iretcd)
{                                                                       
    register int i;
    ...
    *rgx = 0.0;
    *rsx = 0.0;
    TOLSGF = 0.0;       /* final scaled gradient */
    TOLSPF = 0.0;       /* final scaled parameter change */
    ...
    d = dmax(fabs(fpls),fscale);                                          

    for (i = 1; i <= n; ++i) {
        relgrd = fabs(gpls[i]) * dmax(fabs(xpls[i]),1.0) / d;                     
        *rgx = dmax(*rgx,relgrd);                                            
    }
    TOLSGF = *rgx;
    if (PMProtFDef > 1)  
        prval("\nScaled gradient",TOLSGF);
    ...
    for (i = 1; i <= n; ++i) {
        relstp = fabs(xpls[i] - x[i]) / dmax(fabs(xpls[i]),1.0);                  
        *rsx = dmax(*rsx,relstp);                                            
    }
    TOLSPF = *rsx;
    if (PMProtFDef > 1)  
        prval("Scaled parameter change",TOLSPF);
...
int ts_func(int n,double *x,double *f,int gflg,double *g,int hflg,double *h)
{
    ...
    if (TSFNC == 0) {       /* standard models, use fn() */
        m = 0;
        if (gflg)
            m++;
        if (hflg)
        ...
        *f = FTmp;
        ...
        m = get_flval(f,n,x + 1,deriv,1,g,h,h);  
        if (m) {
            *f = 0.0;
            return(m);
        }
...
void ts_prn(int iter,double f,double gnorm,double pc,int n,double *x,double *g)
{
    ...
    if (SILENTFlg < 2) {
        printfe("  %3d  %20.13e %17.10e ",iter,f,gnorm);
        printfe("%11.4e  %6d (%d,%d)\n",pc,NumF,NumFG,NumFH);
    }
    if (PMProtFDef) {
        if (PMProtFDef > 1)  
            fprintf(PMProtFd,"\nITER      Function Value     Norm of Gradient  Par Change   FCall\n");
        fprintf(PMProtFd,"  %3d  %20.13e %17.10e ",iter,f,gnorm);
...
void ts_prn1(int n,double *h)
{
    ...
    if (PMProtFDef > 1) {
        fprintf(PMProtFd,"Hessian\n");
        for (i = 1; i <= n; ++i) {
            for (j = 1; j <= n; ++j)
                fprintf(PMProtFd,PMPFmtS,h[(i - 1) * n + j]);
            fprintf(PMProtFd,"\n");
        }
    }
}
